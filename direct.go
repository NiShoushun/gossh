...
// CopyBufferExtraWithContext 导出的 io.copyBuffer 函数，可传入 Context 对应的 cancelFunc 来终止流之间的复制
func CopyBufferExtraWithContext(dst io.Writer, src io.Reader, extra io.Writer, buf []byte, ctx context.Context) (written int64, err error) {
	// If the reader has a WriteTo method, use it to do the copy.
	// Avoids an allocation and a copy.
	if wt, ok := src.(io.WriterTo); ok {
		return wt.WriteTo(dst)
